...
def s4d_kernel_zoh(C, A, L, step):
    def kernel_l(ldx): return (
        C * (np.exp(step*A)-1)/A * np.exp(ldx*step*A)).sum()
    # def kernel_l(ldx):
    #     return step / (1 + ldx)
    ...
    def setup(self):
        ...
        if self.scaling == "hippo":
            ...
            def arange_initializer(scale):
                return lambda key, sh: scale * np.ones(sh) * np.arange(sh[-1])
            self.A_im = self.param(
                "A_im", arange_initializer(np.pi), (self.N,))
        else:
