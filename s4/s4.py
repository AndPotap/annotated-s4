...
def K_conv(Ab, Bb, Cb, L):
    return np.array(
        [(Cb @ matrix_power(Ab, ldx) @ Bb).reshape() for ldx in range(L)]
    )
...
def K_gen_inverse(Ab, Bb, Cb, L):
    Id = np.eye(Ab.shape[0])
    Ab_L = matrix_power(Ab, L)
    Ct = Cb @ (Id - Ab_L)
    return lambda z: (Ct.conj() @ inv(Id - Ab * z) @ Bb).reshape()
...
def cauchy(v, omega, lambd):
    def cauchy_dot(_omega): return (v / (_omega - lambd)).sum()
...
def test_gen_dplr(L=16, N=4):
    ...
    C = (Id - matrix_power(Ab, L)).conj().T @ Cb.ravel()
...
class S4Layer(nn.Module):
    ...
    def setup(self):
        ...
        self.Lambda = np.clip(self.Lambda_re, None, -
                              1e-4) + 1j * self.Lambda_im
        ...
        self.D = self.param("D", nn.initializers.ones, (1,))
        self.step = np.exp(self.param(
            "log_step", log_step_initializer(), (1,)))
